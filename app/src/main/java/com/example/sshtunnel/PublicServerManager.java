...
final class PublicServerManager {
    ...
    private static String claimPython() {
        return String.join("\n",
                ...
                "pruned=[]",
                "for u in users:",
                " if u.get('public_pool')==pool and u.get('expires') and u['expires']<today:",
                "  try: subprocess.run(['userdel','-r',u['login']],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)",
