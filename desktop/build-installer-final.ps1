...
try {
    ...
    & $Python -m PyInstaller `
        ...
        --windowed `
        --distpath "$ReleaseDir" `
        --name "PelmeniVPN-Windows-Setup-1.40-beta2" `
    ...
    Copy-Item (Join-Path $ReleaseDir "PelmeniVPN-Windows-Setup-1.40-beta2.exe") (Join-Path $RootDir "PelmeniVPN-Windows-Setup-1.40-beta2.exe") -Force
} finally {
    Pop-Location
}
