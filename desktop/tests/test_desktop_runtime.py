...
class DesktopRuntimeTests(unittest.TestCase):
    ...
    def test_tun_helper_imports_and_validates_request(self) -> None:
        from pelmeni_desktop.tun_broker import run_tun_helper
        self.assertEqual(run_tun_helper("non-existent-path.json"), 2)
    ...
    def test_private_vpn_and_public_telegram_ports_are_separate(self) -> None:
        ...
        with patch.object(tunnel_module, "_connect_socket", return_value=connection_socket), patch.object(
            ...
            self.assertFalse(client.connect.call_args.kwargs.get("compress", True))
    ...
    def test_latency_uses_lightweight_keepalive_or_proxy_metric(self) -> None:
        transport = Mock()
        transport.is_active.return_value = True
        client = Mock()
        client.get_transport.return_value = transport
