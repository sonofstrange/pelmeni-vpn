...
final class ServerAccessManager {
    ...
    private static String workerPython() {
        return String.join("\n",
                ...
                "  pool=req.get('pool','')",
                "  to_delete=[x['login'] for x in users if x.get('login','').startswith('pel_pub_') or bool(x.get('public_pool')) or x.get('label','').startswith('Public ')]",
                "  try:",
                "   for p in pwd.getpwall():",
                "    if p.pw_name.startswith('pel_pub_') and p.pw_name not in to_delete:",
                "     to_delete.append(p.pw_name)",
                "  except Exception: pass",
                "  users[:]=[x for x in users if x.get('login') not in to_delete]",
                "  for ulog in to_delete:",
                "   subprocess.run(['pkill','-9','-u',ulog],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)",
                "   subprocess.run(['userdel','-f','-r',ulog],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)",
                "  save()",
                "  try:",
                "   usage=json.load(open(usage_path,encoding='utf-8'))",
                "   for ulog in to_delete: usage.pop(ulog,None)",
                "   save_json(usage_path,usage)",
                "  except Exception: pass",
                "  try:",
                "   if os.path.exists('/etc/pelmeni-vpn/public-claims.json'):",
                "    os.remove('/etc/pelmeni-vpn/public-claims.json')",
                "  except Exception: pass",
                " elif action=='import':",
