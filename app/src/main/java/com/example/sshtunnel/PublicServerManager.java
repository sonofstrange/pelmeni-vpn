...
final class PublicServerManager {
    ...
    private static String claimPython() {
        return String.join("\n",
                ...
                "now=int(time.time()); today=str(datetime.date.today())",
                "users_path='/etc/pelmeni-vpn/users.json'",
                "try: users=json.load(open(users_path,encoding='utf-8'))",
                "except Exception: users=[]",
                "pruned=[]",
                "for u in users:",
                " is_pub = u.get('public_pool')==pool or u.get('login','').startswith('pel_pub_')",
                " issued = int(u.get('issued_at', 0))",
                " is_expired = (u.get('expires') and u['expires']<today) or (is_pub and issued > 0 and now - issued > 30*86400)",
                " if is_pub and is_expired:",
                "  try:",
                "   subprocess.run(['pkill','-9','-u',u['login']],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)",
                "   subprocess.run(['userdel','-f','-r',u['login']],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)",
                "  except Exception: pass",
                " else: pruned.append(u)",
                "users=pruned",
                "ckey=pool+':'+client; existing_claim=claims.get(ckey)",
                "if existing_claim and isinstance(existing_claim,dict):",
                " elog=existing_claim.get('login'); ecode=existing_claim.get('code')",
                " if elog and ecode and any(u.get('login')==elog for u in users):",
                "  print('PEL_PUBLIC_CODE='+ecode); sys.exit(0)",
                "elif existing_claim and isinstance(existing_claim,(int,float)) and now-int(existing_claim)<30:",
                " fail('Повтори выдачу через минуту.')",
                "try:",
                " p=subprocess.run(['pgrep','-c','-f','sshd: pel_pub_'],capture_output=True,text=True)",
                " active_conn=int(p.stdout.strip() or '0')",
                "except Exception: active_conn=0",
                "max_conn=int(cfg.get('max_users',50))",
                "if max_conn>0 and active_conn>=max_conn: fail('Сервер временно перегружен (максимум одновременных подключений). Попробуй через пару минут.')",
                ...
                "claims[ckey]={'time':now,'login':login,'code':u['access_code']}; open(claims_path,'w').write(json.dumps(claims)); os.chmod(claims_path,0o600)",
