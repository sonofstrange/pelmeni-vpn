        ...
        StackLayout {
            ...
            ScrollView {
                ...
                Column {
                    ...
                    ActionCard {
                        width: parent.width
                        title: backend.isServiceInstalled() ? "Служба VPN установлена" : "Установить службу VPN"
                        subtitle: backend.isServiceInstalled()
                            ? "Включение и выключение VPN работает без окон UAC (нажми для удаления)"
                            : "Установить фоновую службу для мгновенного включения VPN без UAC"
                        onClicked: backend.isServiceInstalled() ? backend.uninstallService() : backend.installService()
                    }
