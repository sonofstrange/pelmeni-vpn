...
class ElevatedTunManager:
    ...
    def is_active(self) -> bool:
        if self._direct.is_active():
            return True
        if self._service_client.is_active():
            return True
    ...
    def start(
        ...
        # 1. If background Windows Service is available, start without any UAC prompts!
        if ServiceClient.is_service_available():
            self._service_client.start_vpn(
        ...
        # 2. Otherwise, fallback to elevated helper process
        port = int(socks_port)
        if not 1 <= port <= 65535:
